...
class SettingController extends Controller
{
 ...
 private function navigation(array $value):array{if(count($value)>8)throw ValidationException::withMessages(['value'=>'Navigation supports up to eight links.']);return collect($value)->map(function($item){if(!is_array($item)||array_diff(array_keys($item),['label','path'])||empty($item['label'])||empty($item['path'])||mb_strlen($item['label'])>40||!preg_match('#^/[a-z0-9/_-]*$#',$item['path']))throw ValidationException::withMessages(['value'=>'Each navigation item needs a short label and safe internal path.']);return['label'=>strip_tags($item['label']),'path'=>$item['path']];})->all();}
