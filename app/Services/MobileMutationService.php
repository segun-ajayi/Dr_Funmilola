...
class MobileMutationService
{
    public function run(User $user, string $requestId, string $operation, callable $callback): array
    {
        return DB::transaction(function () use ($user, $requestId, $operation, $callback) {
            $existing = MobileMutation::where('user_id', $user->id)
