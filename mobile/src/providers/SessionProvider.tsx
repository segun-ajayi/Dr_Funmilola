...
type Value={session:Session|null;restoring:boolean;signIn(email:string,password:string):Promise<void>;signOut():Promise<void>};
...
export function SessionProvider({children}:PropsWithChildren){
 ...
 const value=useMemo(()=>({session,restoring,signIn,signOut}),[session,restoring,signIn,signOut]);return <Context.Provider value={value}>{children}</Context.Provider>;
}
