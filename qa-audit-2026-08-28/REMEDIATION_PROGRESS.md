...
- Active slice: S4 actual-page navigation editing and page lifecycle. Source of truth: `evidence/QA-002/S4_NAVIGATION_PAGES_PLAN.md`; S4.1 revision `d907c7c` delivers rendered-header selection, safe internal/external navigation, root/submenu lifecycle, private draft save and explicit publication. S4.2 revision `626f9d3` delivers safe blank/template creation from the actual-site toolbar and opens the protected draft at its canonical `/p/{slug}` route. Fresh gate: backend 114 tests/1,005 assertions, web 10 files/39 tests, production build and diff check pass. S4.3 combined persistence/publication evidence is active.
