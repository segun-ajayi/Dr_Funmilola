...
- Current implementation assessment: the actual-page toolbar, atomic document lifecycle, cursor-in-rendered-element text editing, live typography/rendered-button/linked-text controls and the complete 18-component responsive hierarchy are implemented. Actual-page navigation/page lifecycle, persistent managed media, direct-rollback UI and uninterrupted release journeys remain incomplete.
...
- Completed slice: S3 expanded sections, 18-component catalogue, container hierarchy and responsive presentation. The uninterrupted release journeys remain S7 evidence and QA-002/QA-003 remain REOPENED.
- S3 source of truth: `evidence/QA-002/S3_SECTIONS_COMPONENTS_PLAN.md`. S3.1 revision `a894ed9` delivers section/responsive lifecycle controls; S3.2 revision `189d4bb` delivers the searchable 18-component actual-page library; S3.3 revision `442f98e` delivers bounded live editing and lifecycle controls for all eight repeatable component types plus the visible Page › Section › Component › Element hierarchy; S3.4 revision `f284f6e` verifies private save/reload, exact preview, logged-out publish and previous-version rollback for simple, responsive and nested content. Fresh gate: backend 111 tests/937 assertions, web 10 files/37 tests, production build and diff check pass.
- Completed slice: S4 actual-page navigation editing and page lifecycle. Source of truth: `evidence/QA-002/S4_NAVIGATION_PAGES_PLAN.md`; S4.1 revision `d907c7c` delivers rendered-header selection, safe internal/external navigation, root/submenu lifecycle, private draft save and explicit publication. S4.2 revision `626f9d3` delivers safe blank/template creation from the actual-site toolbar and opens the protected draft at its canonical `/p/{slug}` route. S4.3 revision `92c5597` proves the combined private page/navigation journey, exact preview and independent explicit publication boundaries. Fresh gate: backend 115 tests/1,037 assertions, web 10 files/39 tests, production build and diff check pass. QA-002 and QA-003 remain REOPENED pending the remaining VE rows and independent release evidence.
- Completed slice: S5 actual-page media and image editing. Source of truth: `evidence/QA-002/S5_MEDIA_IMAGES_PLAN.md`; revisions `7b23d96`, `d6cdb02`, `35cb775` and `c0d80ec`. The slice now covers the secure persistent library/scanner boundary, exact rendered Image/Text + Image/Gallery selection, complete live image controls, reusable responsive backgrounds, recoverable upload and keyboard focus, real draft save/reload, exact preview, logged-out isolation, atomic publication, retained version and direct rollback continuity. Fresh gate: focused media 9 tests/162 assertions, backend 124 tests/1,200 assertions, web 10 files/43 tests, PHP formatting, TypeScript, production build, diff check and disposable SQLite fresh migrate/full rollback/reapply pass. VE-048–065 and VE-206–214 are implemented and ready for independent row-level retest; VE-233 and final release rows are not self-approved. QA-002 and QA-003 remain REOPENED.
- Active slice: S6 editor resilience, accessibility and viewport hardening. Source of truth: `evidence/QA-002/S6_RESILIENCE_ACCESSIBILITY_PLAN.md`; planning revision `211e7a5`, S6.1 implementation `575cd5d`. S6.1 is complete: single-flight save/publication, retained typing during requests, truthful failures, Retry Save, same-account expired-session recovery, opt-in idle autosave, bounded account/page/tab-local restore/discard, explicit concurrent-edit server review and protected unload. Fresh gate: backend 124 tests/1,200 assertions, web 12 files/65 tests, focused recovery 22 tests, TypeScript, production build, diff check and changed-file PHP formatting pass. Whole-repository PHP style debt remains in unchanged files; no PHP/database changes were made. VE-199–203 and VE-205 are implemented and ready for independent retest. S6.2 keyboard/modal/viewport work is next, followed by S6.3 full failure-matrix/audit evidence. QA-002 and QA-003 remain REOPENED and final release rows are not self-approved.
