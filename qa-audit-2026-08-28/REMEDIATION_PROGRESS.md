...
- Active slice: S5 actual-page media and image editing. Source of truth: `evidence/QA-002/S5_MEDIA_IMAGES_PLAN.md`; plan revision `df85272`. S5.1 revision `7b23d96` delivers the Power-Admin-only persistent library, quarantine/scanning pipeline, searchable reusable metadata, private/published delivery boundary, reference-safe archive behavior and fail-closed CMS publication validation. S5.2 revisions `d6cdb02` and `35cb775` deliver exact rendered Image, Text + Image and Gallery selection, persistent library reuse/upload with retained failure state, live accessible image controls, reusable responsive section backgrounds and modal focus return. Fresh gate: backend 123 tests/1,138 assertions, web 10 files/42 tests, TypeScript, production build and diff check pass; the S5.1 fresh SQLite migrate/rollback/reapply evidence remains current because S5.2 adds no database migration. S5.3 private reload/preview/publish/version/rollback continuity evidence is active. QA-002 and QA-003 remain REOPENED.
