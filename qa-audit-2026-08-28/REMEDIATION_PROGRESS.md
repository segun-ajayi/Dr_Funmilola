...
- Active slice: S5 actual-page media and image editing. Source of truth: `evidence/QA-002/S5_MEDIA_IMAGES_PLAN.md`; plan revision `df85272`. S5.1 revision `7b23d96` delivers the Power-Admin-only persistent library, quarantine/scanning pipeline, searchable reusable metadata, private/published delivery boundary, reference-safe archive behavior and fail-closed CMS publication validation. Fresh gate: backend 123 tests/1,137 assertions, web 10 files/39 tests, production build, diff check and fresh SQLite migrate/rollback/reapply pass. S5.2 exact rendered-image interaction is active.
