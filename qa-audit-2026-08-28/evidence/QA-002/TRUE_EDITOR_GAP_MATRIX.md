...
| VE-199 | IMPLEMENTED — READY FOR RETEST | S6 | Autosave runs periodically if enabled. | S6.1: explicit account-specific opt-in; saves after 15 seconds of idle editing; single-flight, recovery and conflict guards; pauses after failure and never publishes. Actual-page fake-clock regression passes. |
| VE-200 | IMPLEMENTED — READY FOR RETEST | S6 | The editor clearly indicates unsaved work. | S6.1: current-document/baseline comparison, live unsaved status, protected local copy, Undo-to-baseline cleanup and unload/exit protection. Newer edits during save, preview or publication are not shown as saved. |
| VE-201 | IMPLEMENTED — READY FOR RETEST | S6 | The editor clearly indicates save-in-progress and save-success states. | S6.1: single-flight busy/saving/saved state, disabled overlapping mutations, and exact confirmation of submitted versus newer local edits. Actual-page in-flight typing regressions pass. |
| VE-202 | IMPLEMENTED — READY FOR RETEST | S6 | Save failure is clearly shown and never reported as success. | S6.1: safe network, session, permission, conflict, validation, throttling and server error classification clears stale success. Unconfirmed publication is never claimed to have failed before server acceptance. Full operation cross-product remains S6.3. |
| VE-203 | IMPLEMENTED — READY FOR RETEST | S6 | A save failure preserves recoverable local edits and offers retry or manual save. | S6.1: Retry Save, same-account session recovery, bounded 24-hour tab-local copy, explicit restore/discard, storage failure warning and retained Undo/Redo. Interrupted-save and fresh-session recovery regressions pass. |
...
| VE-205 | IMPLEMENTED — READY FOR RETEST | S6 | Concurrent Power Admin edits are conflict-detected or safely merged; silent overwrites do not occur. | Existing backend lock-version rejection plus S6.1 actual-page 409 recovery: stale resubmission blocked; latest server draft reviewed without losing the local copy; older-copy restoration requires explicit whole-draft replacement confirmation. Independent two-browser retest remains. |
