...
Route::middleware('mobile.envelope')->group(function(){
 ...
 Route::middleware(['auth:sanctum','active','verified','abilities:mobile:v1','role:patient'])->group(function(){
  Route::delete('/auth/token',[MobileAuthController::class,'destroy']);Route::get('/me',[MobilePatientController::class,'me']);Route::get('/appointments',[MobilePatientController::class,'appointments']);Route::get('/documents',[MobilePatientController::class,'documents']);Route::get('/message-threads',[MobilePatientController::class,'threads']);Route::get('/notifications',[MobilePatientController::class,'notifications']);Route::get('/consultations',[MobilePatientController::class,'consultations']);Route::get('/appointments/{appointment}/reschedule-options',[MobileMutationController::class,'rescheduleOptions'])->middleware('throttle:60,1');Route::post('/appointments/{appointment}/cancellation-requests',[MobileMutationController::class,'cancellation'])->middleware('throttle:20,1');Route::post('/appointments/{appointment}/reschedule-requests',[MobileMutationController::class,'reschedule'])->middleware('throttle:20,1');Route::post('/devices/push-token',[MobilePatientController::class,'registerPush']);
 });
});
