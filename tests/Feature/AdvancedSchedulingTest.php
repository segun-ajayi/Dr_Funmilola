...
class AdvancedSchedulingTest extends TestCase
{
    ...
    {
        $staff=User::factory()->create(['role'=>UserRole::Admin]);$service=$this->service();$date=CarbonImmutable::now('Africa/Lagos')->addWeek()->startOfDay();
        ...
        $slots=app(AvailabilityService::class)->slots($service,$date,'in_person');$this->assertTrue($slots->contains(fn($slot)=>str_contains($slot['label'],'5:00 PM')));
    }
    ...
    public function test_only_staff_can_manage_audited_schedule_exceptions(): void
    {
        ...
        Sanctum::actingAs(User::factory()->create());$this->postJson('/api/staff/availability-exceptions',$payload)->assertForbidden();
        $staff=User::factory()->create(['role'=>UserRole::Moderator]);Sanctum::actingAs($staff);$response=$this->postJson('/api/staff/availability-exceptions',$payload)->assertCreated();
    ...
    {
        $patient=User::factory()->create();$patient->notificationPreference()->create(['in_app_reminders'=>true,'email_reminders'=>false,'push_reminders'=>false]);$this->appointment($patient,now()->addHour());
        Artisan::call('appointments:send-reminders');Artisan::call('appointments:send-reminders');
        $this->assertDatabaseCount('notification_deliveries',2);$this->assertDatabaseCount('notifications',2);
    ...
    {
        $staff=User::factory()->create(['role'=>UserRole::PowerAdmin]);$patient=User::factory()->create();$appointment=$this->appointment($patient,now()->addWeek());Sanctum::actingAs($staff);
    ...
    private function service(): Service { return Service::create(['name'=>'Review','slug'=>'advanced-review-'.Str::random(5),'summary'=>'Specialist review','duration_minutes'=>45]); }
    private function appointment(User $patient,$start): Appointment { $service=$this->service();return Appointment::create(['public_id'=>Str::uuid(),'patient_id'=>$patient->id,'service_id'=>$service->id,'starts_at'=>$start,'ends_at'=>$start->copy()->addMinutes(45),'status'=>'confirmed','consultation_method'=>'in_person','reason'=>'Review']); }
}
